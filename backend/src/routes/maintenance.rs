...
pub async fn create_maintenance(
    ...
    let comp_exists: Option<(i64,)> = sqlx::query_as(
        "SELECT id FROM components WHERE id = $1 AND company_id = $2",
    )
    ...
    let (record_id,): (i64,) = sqlx::query_as(
        "INSERT INTO maintenance_records (component_id, technician_id, maintenance_type, description, parts_replaced, inspection_result, record_hash, created_at, company_id) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9) RETURNING id",
    )
    .bind(req.component_id)
    .bind(user.0.sub)
    ...
    Ok((
        ...
        Json(MaintenanceRecordResponse {
            ...
            technician_name: user.0.name.clone(),
...
pub async fn list_maintenance(
    ...
    for r in records {
        let tech_name: Option<(String,)> = sqlx::query_as(

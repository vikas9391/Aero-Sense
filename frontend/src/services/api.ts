...
export const companiesApi = {
  list: async () => (await api.get<CompanySummary[]>('/companies')).data,
  getById: async (id: number) => (await api.get<CompanySummary>(`/companies/${id}`)).data,
  create: async (data: { name: string; slug?: string }) => (await api.post<Company>('/companies', data)).data,
  createAdmin: async (companyId: number, data: { name: string; email: string; password: string }) => (await api.post<User>(`/companies/${companyId}/admins`, data)).data,
  getAnalytics: async (id: number) => (await api.get<WorkAnalytics>(`/companies/${id}/analytics`)).data,
  listUsers: async (id: number) => (await api.get<User[]>(`/companies/${id}/users`)).data,
...
export const componentsApi = {
  ...
  create: async (data: { aircraft_id?: number | null; serial_number: string; component_type: string; manufacturer: string; status?: string }) => (await api.post<Component>('/components', data)).data,
...
export const verificationApi = {
  ...
  listLogs: async () => (await api.get<VerificationLog[]>('/verification/logs')).data,
};

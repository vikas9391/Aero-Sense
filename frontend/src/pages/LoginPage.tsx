...
export const LoginPage: React.FC = () => {
  ...
  return (
    <div className="relative h-screen min-h-0 overflow-hidden bg-[#f7f8ff] text-slate-900">
      ...
      <main className="relative z-10 mx-auto flex h-screen w-full max-w-7xl items-center justify-center px-5 py-4 sm:px-8 lg:px-12">
        <div className="w-full max-w-5xl max-h-[calc(100vh-32px)] rounded-[34px] border border-white/80 bg-white/55 p-2 shadow-[0_40px_120px_rgba(51,52,105,.14)] backdrop-blur-2xl sm:p-3">
          <div className="grid max-h-[calc(100vh-56px)] overflow-hidden rounded-[27px] border border-indigo-100/80 bg-white/90 shadow-[0_15px_55px_rgba(68,76,150,.08)] lg:grid-cols-[1fr_450px]">
            <section className="hidden p-10 lg:flex lg:flex-col xl:p-12">
              <AeroLogo size="md" />
              <div className="max-w-xl pt-16 pb-4 xl:pt-14">
                <div className="mb-5 inline-flex items-center gap-2 rounded-full border border-indigo-200 bg-indigo-50 px-3 py-1.5 text-[10px] font-extrabold uppercase tracking-[.18em] text-indigo-700"><span className="h-1.5 w-1.5 rounded-full bg-emerald-500" /> Aviation intelligence platform</div>
                ...
                <p className="mt-6 max-w-lg text-base leading-7 text-slate-500">Connect physical component identity, NFC verification and maintenance history through one secure operational command center.</p>
                <div className="mt-8 grid max-w-xl grid-cols-3 gap-3">
                  {[{ icon: ScanLine, title: 'Verify', text: 'NFC identity' }, { icon: Database, title: 'Trace', text: 'Lifecycle records' }, { icon: ShieldCheck, title: 'Protect', text: 'Audit integrity' }].map(({ icon: Icon, title, text }) => (
                    <div key={title} className="rounded-2xl border border-slate-200/80 bg-white p-4 shadow-sm"><Icon className="h-4 w-4 text-indigo-600" /><div className="mt-5 text-sm font-bold text-slate-900">{title}</div><div className="mt-1 text-[10px] uppercase tracking-wider text-slate-400">{text}</div></div>
                  ))}
                </div>
              </div>
              <div className="mt-auto flex justify-between pt-4 text-[9px] font-bold uppercase tracking-[.18em] text-slate-400"><span>Secure component intelligence</span><span>AERO-SENSE / 2026</span></div>
            </section>

            <section className="border-indigo-100 bg-gradient-to-b from-white to-indigo-50/35 p-6 sm:p-9 lg:border-l lg:p-9 xl:p-10">
              <div className="mb-6 lg:hidden"><AeroLogo size="md" /></div>
              <div className="mb-6">
                ...
                <p className="mt-1.5 text-sm leading-6 text-slate-500">Sign in to your AeroSense command center.</p>
              </div>
              ...
              <div className="my-5 flex items-center gap-3"><div className="h-px flex-1 bg-slate-200" /><span className="text-[9px] font-bold uppercase tracking-[.16em] text-slate-400">or</span><div className="h-px flex-1 bg-slate-200" /></div>
              ...
              <div className="mt-6 space-y-3 border-t border-slate-200 pt-5"><div className="flex items-center justify-between text-[10px] font-bold uppercase tracking-[.13em] text-slate-400"><span className="flex items-center gap-1.5"><ShieldCheck className="h-3.5 w-3.5 text-emerald-500" /> Protected access</span><span>Encrypted session</span></div><button type="button" onClick={() => navigate('/')} className="w-full text-center text-[10px] font-bold uppercase tracking-[.13em] text-slate-400 transition hover:text-indigo-600">Back to AeroSense</button></div>
            </section>
          </div>
        </div>
      </main>

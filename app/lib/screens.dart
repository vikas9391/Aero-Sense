    ...
    return RefreshIndicator(
      ...
      child: ListView(
        ...
        children: [
          ...
          const SizedBox(height: 4),
          const Text(
            'Fleet and component intelligence at a glance.',
...
class _ComponentsState extends State<ComponentsScreen> {
  ...
  Widget build(BuildContext context) {
    final filtered = items
        .where(
          (x) => '${x.serial} ${x.type} ${x.manufacturer} ${x.status}'
              .toLowerCase()
...
class _PassportState extends State<PassportScreen> {
  ...
  Future<void> load() async {
    try {
      ...
      maintenance = m
          .where(
            (x) => x.componentId == widget.component.id,
          )
          .toList();

      checks = await api.componentVerifications(
        widget.component.id,
      );
    } catch (_) {}
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      backgroundColor: bg,
      appBar: AppBar(
        title: const Text('Component passport'),
        backgroundColor: bg,
      ),
      body: ListView(
    ...
    try {
      ...
      await NfcManager.instance.startSession(
        ...
        onDiscovered: (tag) async {
          try {
// Use the PUBLIC Android API from nfc_manager 4.2.1.
            final androidTag = NfcTagAndroid.from(tag);

            if (androidTag == null) {
              throw Exception(
                'Unable to read the Android NFC tag.',
              );
            }

// NfcTagAndroid.id is already a Uint8List.
            final bytes = androidTag.id;

            if (bytes.isEmpty) {
              throw Exception(
                'Unable to read the NFC tag identifier.',
              );
            }
  ...
  Widget build(BuildContext context) {
    return ListView(
      ...
      children: [
        ...
        const SizedBox(height: 4),
        const Text(
          'Authenticate a component using the phone NFC reader.',
          style: TextStyle(color: muted),
        ),
        const SizedBox(height: 18),
        CardBox(
          child: Column(
            children: [
              Container(
                ...
                child: Icon(
                  scanning ? Icons.radar : Icons.nfc_outlined,
                  color: accent,
                  size: 48,
                ),
              ),
              const SizedBox(height: 16),
              Text(
                scanning ? 'HOLD PHONE NEAR TAG' : 'READY TO VERIFY',
                style: const TextStyle(
                  color: accent,
                  fontSize: 10,
                  fontWeight: FontWeight.w800,
              ...
              const SizedBox(height: 8),
              const Text(
                'The physical tag UID is read by Android and sent '
                'to the secure backend for authentication and '
                'component binding.',
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      decoration: BoxDecoration(
        color: r.verified ? const Color(0xFFE8F5F0) : const Color(0xFFFFEEEE),
        ...
        children: [
          ...
          if (r.component != null)
            Padding(
              ...
              child: Text(
                '${r.component!['serial_number'] ?? ''} · '
                '${r.component!['aircraft'] ?? ''}',
                style: const TextStyle(
                  fontWeight: FontWeight.w700,
                ),
              ),
  ...
  Future<void> go() async {
    ...
    try {
      ...
      if (token is! String || token.isEmpty) {
        throw Exception(
          'Login succeeded without a session token.',
        );
      }

...
class _DashboardState extends State<Dashboard> {
  ...
  Widget _stat(String label, int value, IconData icon) => SizedBox(
        ...
        height: 105,
        child: CardBox(child: Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [Icon(icon, color: accent), Text('$value', style: const TextStyle(fontSize: 27, fontWeight: FontWeight.w800)), Text(label, style: const TextStyle(color: muted))])),
      );
}
...
class _ComponentsState extends State<ComponentsScreen> {
  ...
  Future<void> load() async {
    try { items = await api.components(); } catch (_) {}
    if (mounted) setState(() => loading = false);
  }
  ...
  void initState() {
    super.initState();
    load();
    api.me().then((v) { if (mounted) setState(() => user = v); }).catchError((_) {});
  }
    ...
    return RefreshIndicator(
      ...
      child: ListView(
        ...
        children: [
          ...
          if (canRegister) ...[
            Row(children: [
              Expanded(child: OutlinedButton.icon(onPressed: () => _open(const RegisterTagScreen()), icon: const Icon(Icons.link_outlined), label: const Text('Bind NFC / RFID'))),
              const SizedBox(width: 10),
              Expanded(child: FilledButton.icon(onPressed: () => _open(const RegisterComponentScreen()), icon: const Icon(Icons.add_box_outlined), label: const Text('Register'))),
            ]),
            const SizedBox(height: 14),
          ],
          ...
          if (loading)
            ...
            ...filtered.map((x) => CardBox(
                  ...
                  child: ListTile(
                    ...
                    subtitle: Text('${x.manufacturer} · ${x.type}\n${x.aircraftRegistration ?? 'Unassigned'}', style: const TextStyle(color: muted, height: 1.35)),
...
class _PassportState extends State<PassportScreen> {
  ...
  Future<void> load() async {
    try {
      maintenance = (await api.maintenance()).where((x) => x.componentId == widget.component.id).toList();
  ...
  Widget build(BuildContext context) => Scaffold(
        ...
        body: ListView(
          ...
          children: [
            ...
            const SizedBox(height: 12),
            Section(title: 'Verification history', child: loading ? const CircularProgressIndicator(color: accent) : checks.isEmpty ? const Text('No verification records yet.', style: TextStyle(color: muted)) : Column(children: checks.map((x) => EventRow(title: x.status, subtitle: x.createdAt, ok: x.status == 'AUTHENTIC' || x.status == 'PASSED')).toList())),
            const SizedBox(height: 12),
            Section(title: 'Maintenance history', child: loading ? const CircularProgressIndicator(color: accent) : maintenance.isEmpty ? const Text('No maintenance records yet.', style: TextStyle(color: muted)) : Column(children: maintenance.map((x) => EventRow(title: x.type, subtitle: '${x.createdAt} · ${x.technician}', ok: x.result == 'PASSED')).toList())),
          ],
        ),
      );

  Widget _kv(String k, String v) => Padding(padding: const EdgeInsets.only(bottom: 9), child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [Expanded(child: Text(k, style: const TextStyle(color: muted))), Expanded(child: Text(v, style: const TextStyle(fontWeight: FontWeight.w700)))]));
}

class ActivityScreen extends StatefulWidget {
  const ActivityScreen({super.key});
  @override State<ActivityScreen> createState() => _ActivityState();
}
...
class _ActivityState extends State<ActivityScreen> {
  ...
  Future<void> load() async {
    try { checks = await api.verificationLogs(); maintenance = await api.maintenance(); } catch (_) {}
    if (mounted) setState(() => loading = false);
  }
  ...
  Widget build(BuildContext context) {
    final rows = <Widget>[
      ...checks.map((x) => EventRow(title: 'Verification · ${x.status}', subtitle: x.createdAt, ok: x.status == 'AUTHENTIC' || x.status == 'PASSED')),
      ...maintenance.map((x) => EventRow(title: 'Maintenance · ${x.type}', subtitle: '${x.createdAt} · ${x.technician}', ok: x.result == 'PASSED')),
    ];
    return RefreshIndicator(onRefresh: load, child: ListView(padding: const EdgeInsets.fromLTRB(20, 20, 20, 100), children: [
      ...
      const SizedBox(height: 5), const Text('Activity', style: TextStyle(fontSize: 28, fontWeight: FontWeight.w800)),
      const SizedBox(height: 15),
...
class _ProfileState extends State<ProfileScreen> {
  ...
  Widget build(BuildContext context) => ListView(
        ...
        children: [
          ...
          const SizedBox(height: 5), const Text('Profile', style: TextStyle(fontSize: 28, fontWeight: FontWeight.w800)), const SizedBox(height: 16),
          CardBox(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            const CircleAvatar(radius: 28, backgroundColor: Color(0xFFECECF8), child: Icon(Icons.person_outline, color: accent)),
            const SizedBox(height: 14), Text(user?.name ?? 'Loading…', style: const TextStyle(fontSize: 21, fontWeight: FontWeight.w800)),
            Text(user?.email ?? '', style: const TextStyle(color: muted)), const SizedBox(height: 15), StatusPill(user?.role ?? '—'), const SizedBox(height: 14),
          ...
          const SizedBox(height: 18),
          OutlinedButton.icon(onPressed: () async { await api.storage.delete(key: tokenKey); if (context.mounted) Navigator.of(context).pushAndRemoveUntil(MaterialPageRoute(builder: (_) => const LoginScreen()), (_) => false); }, icon: const Icon(Icons.logout), label: const Text('Sign out')),
        ],
      );
}
...
class _LoginState extends State<LoginScreen> {
  ...
  Widget build(BuildContext context) => Scaffold(
        backgroundColor: bg,
        body: SafeArea(child: Center(child: SingleChildScrollView(padding: const EdgeInsets.all(24), child: ConstrainedBox(constraints: const BoxConstraints(maxWidth: 520), child: CardBox(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          const Icon(Icons.flight_takeoff_rounded, color: accent, size: 42),
          const SizedBox(height: 18), const Text('AERO-SENSE', style: TextStyle(fontSize: 28, fontWeight: FontWeight.w900, letterSpacing: 1.4)),
          const SizedBox(height: 4), const Text('Secure aircraft component intelligence', style: TextStyle(color: muted)), const SizedBox(height: 25),
...
class _AppShellState extends State<AppShell> {
  ...
  bool get isMaintenance => isCompanyAdmin || role == 'MAINTENANCE_TECHNICIAN';
  ...
  List<_NavItem> get items {
    ...
    return <_NavItem>[
      ...
      const _NavItem('Components', Icons.inventory_2_outlined),
      if (isMaintenance) const _NavItem('Maintenance', Icons.build_outlined),
      if (isCompanyAdmin) const _NavItem('Users', Icons.group_outlined),
      if (isCompanyAdmin) const _NavItem('Analytics', Icons.insights_outlined),
      const _NavItem('Activity', Icons.security_outlined),
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text(nav[index].label, style: const TextStyle(fontWeight: FontWeight.w800)),
          if (user != null) Text(user!.name, style: const TextStyle(fontSize: 11, color: muted)),
        ]),
      ),
      drawer: Drawer(
        child: SafeArea(child: Column(children: [
          const SizedBox(height: 18),
          const ListTile(leading: Icon(Icons.flight_takeoff_rounded, color: accent), title: Text('AERO-SENSE', style: TextStyle(fontWeight: FontWeight.w900, letterSpacing: 1.1)), subtitle: Text('Component intelligence', style: TextStyle(color: muted))),
          const Divider(),
          Expanded(child: ListView.builder(itemCount: nav.length, itemBuilder: (context, i) => ListTile(selected: i == index, leading: Icon(nav[i].icon), title: Text(nav[i].label), onTap: () { setState(() => index = i); Navigator.pop(context); }))),

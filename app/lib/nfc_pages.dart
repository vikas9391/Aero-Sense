    ...
    return ListView(
      ...
      children: [
        ...
        _ActionCard(
          ...
          description: 'Read the NFC UID and immediately see the registered component and tag details.',
        ...
        if (canBind) ...[
          ...
          _ActionCard(
            ...
            onTap: () => context.go('/register-component'),
          ),
          const SizedBox(height: 12),
          _ActionCard(
            icon: Icons.link,
            title: 'Bind NFC tag',
            description: 'Attach a physical NFC tag to an existing aircraft component.',
            button: 'Bind tag',
            onTap: () => context.go('/register-tag'),
          ),
        ],
      ],
    );
        ...
        onDiscovered: (tag) async {
          ...
          if (identifier == null) {
            ...
            if (mounted) {
              setState(() {
                scanning = false;
                error = 'The NFC tag did not expose a readable identifier.';
              });
            }
            return;
          }
        ...
        CardBox(
          child: Column(
            children: [
              ...
              if (uid != null) ...[
                const SizedBox(height: 8),
                Text('UID: $uid', style: const TextStyle(fontFamily: 'monospace', fontWeight: FontWeight.w700)),
              ],
              const SizedBox(height: 14),
              SizedBox(
                width: double.infinity,
                child: FilledButton.icon(
                  ...
                  icon: Icon(scanning ? Icons.hourglass_top : Icons.nfc),
...
class VerificationResultCard extends StatelessWidget {
  ...
  void _bind(BuildContext context) {
    final value = scannedUid;
    if (value == null || value.isEmpty) return;
    context.go('/register-tag?uid=${Uri.encodeComponent(value)}');
  }

  Future<void> _openPassport(BuildContext context) async {
    final databaseId = result.component?['database_id'];
    ...
    try {
      ...
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(nfcApi.errorMessage(e)), backgroundColor: Colors.red));
      }
    }
  }
      ...
      child: Column(
        ...
        children: [
          ...
          if (data != null) ...[
            ...
            const SizedBox(height: 12),
            SizedBox(width: double.infinity, child: FilledButton.icon(onPressed: () => _openPassport(context), icon: const Icon(Icons.badge_outlined), label: const Text('Open full component passport')),
          ] else if (!passed) ...[
            const SizedBox(height: 12),
  ...
  Widget _section(String title, List<Widget> rows) {
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(title, style: const TextStyle(color: accent, fontSize: 10, fontWeight: FontWeight.w900, letterSpacing: 1.1)), const SizedBox(height: 10), ...rows]),
    );
  }
    ...
    return Padding(
      ...
      child: Row(
        ...
        children: [
          ...
          const SizedBox(width: 8),
          Expanded(child: Text('${value ?? '—'}', style: const TextStyle(fontWeight: FontWeight.w700, height: 1.3))),
        ],
      ),
    );
  }

  ...
  Widget build(BuildContext context) => Padding(
        ...
        child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Text(text, style: const TextStyle(color: muted, fontSize: 12, height: 1.4)),
          ])),
        ]),
      );
}
          ...
          if (error != null) ...[
            ...
            CardBox(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              if (error!.toLowerCase().contains('not registered')) ...[
                const SizedBox(height: 12),
                SizedBox(width: double.infinity, child: OutlinedButton.icon(onPressed: () => context.go('/register-tag'), icon: const Icon(Icons.link), label: const Text('Register / bind this tag'))),
              ],
            ])),
          ],
          if (result != null) ...[
  ...
  Widget build(BuildContext context) {
    ...
    return CardBox(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      if (data != null) ...[
        ...
        const SizedBox(height: 12),
        const Text('This NFC tag is not bound to a component yet. Bind it first, then scan again to display the component data.', style: TextStyle(color: muted, height: 1.45)),
        const SizedBox(height: 12),
        SizedBox(width: double.infinity, child: OutlinedButton.icon(onPressed: () => context.go('/register-tag'), icon: const Icon(Icons.link), label: const Text('Bind this NFC tag'))),
      ],

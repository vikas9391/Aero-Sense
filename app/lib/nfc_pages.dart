...
class _ActionCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) => CardBox(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
    ...
    const SizedBox(height: 12), Text(title, style: const TextStyle(fontSize: 18, fontWeight: FontWeight.w800)),
    const SizedBox(height: 6), Text(description, style: const TextStyle(color: muted, height: 1.4)), const SizedBox(height: 14),
...
class _NfcVerificationState extends State<NfcVerificationScreen> {
  ...
  void _bindScannedTag() {
    final value = uid; if (value == null || value.isEmpty) return;
    ...
    children: [
      ...
      if (error != null) ...[
        const SizedBox(height: 12), CardBox(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Builder(builder: (_) => Text(error!, style: const TextStyle(color: Colors.red, height: 1.4))),
...
class _VerificationResultCardState extends State<VerificationResultCard> {
  bool editing = false; bool loadingHistory = false; bool saving = false;
  ...
  Future<void> _loadHistory(int id) async {
    ...
    try { history = await nfcApi.componentUpdateHistory(id); } catch (_) {} finally { if (mounted) setState(() => loadingHistory = false); }
  }

  Future<void> _startEdit() async {
    final data = widget.result.component; final id = data?['database_id'];
    if (id is! num) return;
    setState(() => editing = true);
    try {
      aircraft = await nfcApi.aircraft();
      final current = data?['aircraft_id'];
      selectedAircraft = current?.toString();
    } catch (_) {}
    if (mounted) setState(() {});
  }
  ...
  Future<void> _saveEdit() async {
    ...
    if (serial.text.trim().isEmpty || type.text.trim().isEmpty || manufacturer.text.trim().isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Serial number, component type and manufacturer are required.'))); return;
    }
    ...
    try {
      final updated = await nfcApi.updateComponent(id.toInt(), {
        ...
        'serial_number': serial.text.trim(), 'component_type': type.text.trim(), 'manufacturer': manufacturer.text.trim(), 'status': status,
      });
      widget.result.component?.addAll({
        'serial_number': updated.serial, 'component_type': updated.type, 'manufacturer': updated.manufacturer,
        'status': updated.status, 'aircraft': updated.aircraftRegistration ?? 'Unassigned', 'aircraft_id': updated.aircraftId,
      });
      editing = false;
  ...
  void _bind(BuildContext context) {
    final value = widget.scannedUid; if (value == null || value.isEmpty) return;
    context.go('/register-tag?uid=${Uri.encodeComponent(value)}');
  }
  ...
  Widget build(BuildContext context) {
    final result = widget.result; final data = result.component; final passed = result.verified;
    return CardBox(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      if (data != null) ...[
        ...
        _section('COMPONENT DETAILS', [
          ...
          _detail('Last Updated', data['updated_at']),
        ]),
        const SizedBox(height: 12),
        _section('NFC TAG DETAILS', [_detail('Identifier', data['tag_identifier']), _detail('Technology', data['tag_technology']), _detail('Security', data['tag_security_type']), _detail('Tamper status', data['tag_tamper_status']), _detail('Registered', data['tag_registered_at'])]),
        const SizedBox(height: 12),
        Row(children: [Expanded(child: FilledButton.icon(onPressed: () => _openPassport(context), icon: const Icon(Icons.badge_outlined), label: const Text('Passport'))), const SizedBox(width: 10), Expanded(child: OutlinedButton.icon(onPressed: editing ? null : _startEdit, icon: const Icon(Icons.edit_outlined), label: const Text('Update data')))]),
        if (editing) ...[
          const SizedBox(height: 14),
          _editPanel(),
        ],
  ...
  Widget _editPanel() => Container(width: double.infinity, padding: const EdgeInsets.all(14), decoration: BoxDecoration(color: soft, borderRadius: BorderRadius.circular(16)), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
    ...
    const Text('Saving creates a new immutable history entry with the server timestamp and the authenticated user.', style: TextStyle(color: muted, fontSize: 12, height: 1.35)), const SizedBox(height: 12),
  ...
  Widget _historySection(Object? rawId) {
    ...
    return ExpansionTile(leading: const Icon(Icons.history), title: const Text('Update history', style: TextStyle(fontWeight: FontWeight.w800)), subtitle: Text(history.isEmpty ? 'View timestamped changes' : '${history.length} recorded update${history.length == 1 ? '' : 's'}'), onExpansionChanged: (open) { if (open && id != null && history.isEmpty) _loadHistory(id); }, children: [
      if (loadingHistory) const Padding(padding: EdgeInsets.all(16), child: Center(child: CircularProgressIndicator())),
      ...
      for (final item in history) ListTile(contentPadding: EdgeInsets.zero, leading: const Icon(Icons.update), title: Text('${item.serial} · ${item.type}', style: const TextStyle(fontWeight: FontWeight.w700)), subtitle: Text('${item.manufacturer} · ${item.status}\nUpdated ${item.updatedAt}', style: const TextStyle(color: muted)), isThreeLine: true),
    ]);
  }

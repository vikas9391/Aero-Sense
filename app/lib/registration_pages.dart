...
class _RegisterComponentState extends State<RegisterComponentScreen> {
  ...
  @override
  void dispose() {
    serial.dispose(); type.dispose(); manufacturer.dispose(); super.dispose();
  }

  Future<void> save() async {
    if (serial.text.trim().isEmpty || type.text.trim().isEmpty || manufacturer.text.trim().isEmpty) {
      _msg('Complete all required fields.', true); return;
    }
    ...
    try {
      ...
      if (mounted) {
        serial.clear(); type.clear(); manufacturer.clear();
  ...
  Widget build(BuildContext context) => Scaffold(
    ...
    body: ListView(
      ...
      children: [
        ...
        const Text('Register component', style: TextStyle(fontSize: 28, fontWeight: FontWeight.w800)),
        const SizedBox(height: 5),
        const Text('Create the persistent digital identity used by the component passport and NFC verification.', style: TextStyle(color: muted)),
        ...
        CardBox(child: Column(children: [
          ...
          SizedBox(width: double.infinity, height: 52, child: FilledButton.icon(
            ...
            label: Text(saving ? 'Registering…' : 'Register Component'),
          )),
        ])),
      ],
    ),
...
class RegisterTagScreen extends StatefulWidget {
  ...
  @override State<RegisterTagScreen> createState() => _RegisterTagState();
}

class _RegisterTagState extends State<RegisterTagScreen> {
  List<Component> components = [];
  String? componentId;
  String technology = 'NFC', security = 'TAMPER_PROOF';
  final identifier = TextEditingController();
  bool loading = true, scanning = false, saving = false;
  ...
  Future<void> readNfc() async {
    ...
    try {
      ...
      await NfcManager.instance.startSession(
        ...
        onDiscovered: (tag) async {
          try {
            ...
            if (mounted) setState(() => identifier.text = t.id.map((b) => b.toRadixString(16).padLeft(2, '0')).join(':').toUpperCase());
          } catch (e) {
            if (mounted) _msg(api.errorMessage(e), true);
          } finally {
      ...
      if (mounted) { setState(() => scanning = false); _msg(api.errorMessage(e), true); }
    }
  }

  Future<void> save() async {
    if (componentId == null || identifier.text.trim().isEmpty) {
      _msg('Select a component and provide an identifier.', true); return;
    }
    setState(() => saving = true);
    try {
      ...
      if (mounted) { identifier.clear(); _msg('Tag registered successfully.'); }
    } catch (e) {
      if (mounted) _msg(api.errorMessage(e), true);
    } finally {
      if (mounted) setState(() => saving = false);
  ...
  Widget build(BuildContext context) => Scaffold(
    ...
    body: ListView(
      ...
      children: [
        ...
        const SizedBox(height: 4),
        const Text('Register tag', style: TextStyle(fontSize: 28, fontWeight: FontWeight.w800)),
        const SizedBox(height: 5),
        const Text('Bind a physical NFC/RFID identifier to an existing component.', style: TextStyle(color: muted)),
        const SizedBox(height: 18),
        CardBox(child: Column(children: [
          if (loading)
            const LinearProgressIndicator(color: accent)
            ...
            DropdownButtonFormField<String>(
              ...
              items: components.map((c) => DropdownMenuItem<String>(value: '${c.id}', child: Text('${c.serial} · ${c.type}'))).toList(),
          ...
          const SizedBox(height: 12),
          TextField(controller: identifier, decoration: InputDecoration(labelText: 'Identifier *', hintText: '04:AB:12:...', suffixIcon: IconButton(onPressed: scanning ? null : readNfc, icon: Icon(scanning ? Icons.sync : Icons.nfc)))),
          const SizedBox(height: 12),
          DropdownButtonFormField<String>(

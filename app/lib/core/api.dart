...
class Api {
  Api({String? baseUrl}) {
    dio = Dio(BaseOptions(baseUrl: baseUrl ?? const String.fromEnvironment('API_BASE_URL', defaultValue: defaultApiBase), connectTimeout: const Duration(seconds: 15), receiveTimeout: const Duration(seconds: 15), headers: {'Content-Type': 'application/json'}));
    dio.interceptors.add(InterceptorsWrapper(onRequest: (o, h) async { final t = await storage.read(key: 'aero_sense_token'); if (t != null) o.headers['Authorization'] = 'Bearer $t'; h.next(o); }, onError: (e, h) async { if (e.response?.statusCode == 401) await storage.delete(key: 'aero_sense_token'); h.next(e); }));
  }
  late final Dio dio;
  ...
  Future<VerificationResponse> verifyNfc(String tag, {String? payload}) async => VerificationResponse.fromJson(Map<String, dynamic>.from((await dio.post('/verification/nfc', data: {'tag_identifier': tag, 'payload': payload})).data));
  ...
  List<Map<String, dynamic>> _list(dynamic data) { if (data is List) return data.map((e) => Map<String, dynamic>.from(e as Map)).toList(); if (data is Map && data['data'] is List) return (data['data'] as List).map((e) => Map<String, dynamic>.from(e as Map)).toList(); return []; }
}

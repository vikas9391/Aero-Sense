...
class Api {
  ...
  Future<Map<String, dynamic>> login(
      ...
      Map<String, dynamic>.from((await dio.post('/auth/login', data: {
  ...
  Future<Analytics> analytics() async => Analytics.fromJson(
      Map<String, dynamic>.from((await dio.get('/analytics/overview')).data));
  Future<List<Aircraft>> aircraft() async =>
      _list((await dio.get('/aircraft')).data).map(Aircraft.fromJson).toList();
  Future<List<Component>> components() async =>
      _list((await dio.get('/components')).data)
  ...
  Future<MaintenanceRecord> createMaintenance(
      ...
      MaintenanceRecord.fromJson(Map<String, dynamic>.from(
          (await dio.post('/maintenance', data: data)).data));
  Future<List<VerificationLog>> verificationLogs() async =>
      _list((await dio.get('/verification/logs')).data)
          .map(VerificationLog.fromJson)
          .toList();
  Future<List<VerificationLog>> componentVerifications(int id) async =>
      _list((await dio.get('/components/$id/verification')).data)
          .map(VerificationLog.fromJson)
          .toList();
  Future<VerificationResponse> verifyNfc(String tag, {String? payload}) async =>
      VerificationResponse.fromJson(
          Map<String, dynamic>.from((await dio.post('/verification/nfc', data: {
        'tag_identifier': tag,
        if (payload != null && payload.isNotEmpty) 'payload': payload
      }))
              .data));
  Future<Map<String, dynamic>> verifyBlockchain(int recordId) async =>
      Map<String, dynamic>.from(
          (await dio.post('/blockchain/verify', data: {'record_id': recordId}))
              .data);
  ...
  String errorMessage(Object error) {
    if (error is DioException) {
      ...
      if (error.type == DioExceptionType.connectionError)
        return 'Cannot reach Aero-Sense backend. Check the API address and network.';
      if (error.response?.statusCode == 401)
...
class Analytics {
  ...
  factory Analytics.fromJson(Map<String, dynamic> j) => Analytics(
      users: j['total_users'] ?? 0,
      aircraft: j['total_aircraft'] ?? 0,
      components: j['total_components'] ?? 0,
      maintenance: j['total_maintenance_records'] ?? 0,
      verifications: j['total_verifications'] ?? 0,
      passed: j['verifications_passed'] ?? 0,
      failed: j['verifications_failed'] ?? 0);
}
...
class Component {
  ...
  factory Component.fromJson(Map<String, dynamic> j) => Component(
      id: j['id'] ?? 0,
      uuid: j['component_uuid'] ?? '',
      serial: j['serial_number'] ?? '',
      type: j['component_type'] ?? '',
      manufacturer: j['manufacturer'] ?? '',
      status: j['status'] ?? '',
      aircraftId: j['aircraft_id'],
      aircraftRegistration: j['aircraft_registration']);
}
...
class MaintenanceRecord {
  ...
  factory MaintenanceRecord.fromJson(Map<String, dynamic> j) =>
      MaintenanceRecord(
          id: j['id'] ?? 0,
          componentId: j['component_id'] ?? 0,
          technician: j['technician_name'] ?? '',
          type: j['maintenance_type'] ?? '',
          description: j['description'] ?? '',
          result: j['inspection_result'] ?? '',
          hash: j['record_hash'] ?? '',
          createdAt: j['created_at'] ?? '',
          parts: j['parts_replaced']);
}
...
class VerificationLog {
  ...
  factory VerificationLog.fromJson(Map<String, dynamic> j) => VerificationLog(
      id: j['id'] ?? 0,
      status: j['status'] ?? j['final_result'] ?? '',
      createdAt: j['created_at'] ?? '',
...
class VerificationResponse {
  ...
  factory VerificationResponse.fromJson(
      ...
      VerificationResponse(
          ...
          checks: Map<String, bool>.from((j['checks'] ?? {})
              .map((k, v) => MapEntry(k.toString(), v == true))),
          reason: j['failure_reason'],
          component: j['component'] is Map
              ? Map<String, dynamic>.from(j['component'])
              : null);
}

...
class Api {
  Api({String? baseUrl}) {
    ...
    dio.interceptors.add(InterceptorsWrapper(
      ...
      onResponse: (response, handler) {
        if (kDebugMode) {
          debugPrint('[Aero-Sense] ${response.statusCode} ${response.requestOptions.uri}');
        }
      ...
      onError: (error, handler) async {
        if (kDebugMode) {
          debugPrint('[Aero-Sense] ERROR ${error.type} ${error.response?.statusCode ?? ''} ${error.requestOptions.uri}');
        }
  ...
  Future<User> changePassword(String currentPassword, String newPassword) async =>
      User.fromJson(Map<String, dynamic>.from((await dio.put('/auth/change-password', data: {

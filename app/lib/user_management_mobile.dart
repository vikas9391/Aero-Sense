...
class MobileManagedUser {
  ...
  factory MobileManagedUser.fromJson(Map<String, dynamic> j) => MobileManagedUser(id: j['id'] ?? 0, uuid: j['uuid'] ?? '', name: j['name'] ?? '', email: j['email'] ?? '', role: (j['role'] ?? 'VIEWER').toString().toUpperCase(), status: (j['status'] ?? 'ACTIVE').toString().toUpperCase(), createdAt: j['created_at'] ?? '', companyId: j['company_id']);
}
...
class MobileUserProfile {
  ...
  factory MobileUserProfile.fromJson(Map<String, dynamic> j) => MobileUserProfile(user: MobileManagedUser.fromJson(Map<String, dynamic>.from(j['user'] ?? {})), companyName: j['company_name'], maintenanceCount: j['maintenance_count'] ?? 0, componentUpdateCount: j['component_update_count'] ?? 0);
}
...
class _UserManagementScreenState extends State<UserManagementScreen> {
  ...
  Future<void> load() async {
    ...
    try {
      ...
      users = (responses[0].data is List ? responses[0].data : []).map<MobileManagedUser>((e) => MobileManagedUser.fromJson(Map<String, dynamic>.from(e))).toList();
      ...
      builder: (dialogContext) => StatefulBuilder(
        builder: (dialogContext, setDialog) => AlertDialog(
          ...
          actions: [
            ...
            FilledButton(onPressed: () => Navigator.of(dialogContext).pop(selected), child: const Text('Continue')),
          ],
        ),
      ),
    );
    if (role == null || role == user.role) return;
    if (!await _confirm('Change ${user.name}\'s role?', 'This account will immediately use ${_roleLabel(role)} permissions on its next authenticated request.')) return;
    try {
  ...
  Future<void> setStatus(MobileManagedUser user, String status) async {
    ...
    if (!await _confirm('$label ${user.name}?', status == 'DELETED' ? 'The account will be soft-deleted. Work history and audit records are preserved.' : status == 'SUSPENDED' ? 'The account will no longer be able to sign in.' : 'The account will be allowed to sign in again.')) return;
    try {
  ...
  Future<void> viewProfile(MobileManagedUser user) async {
    try {
      ...
      showDialog(context: context, builder: (dialogContext) => AlertDialog(
        ...
        content: SingleChildScrollView(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [_detail('Email', p.user.email), _detail('Role', _roleLabel(p.user.role)), _detail('Status', p.user.status), _detail('Company', p.companyName ?? 'Platform'), const Divider(height: 24), _detail('Maintenance records', '${p.maintenanceCount}'), _detail('Component updates', '${p.componentUpdateCount}')])),
  ...
  Widget _menu(MobileManagedUser user) => PopupMenuButton<String>(
    ...
    itemBuilder: (_) => [
      ...
      if (user.status == 'ACTIVE') const PopupMenuItem<String>(value: 'suspend', child: Text('Suspend account')),
      if (user.status == 'SUSPENDED') const PopupMenuItem<String>(value: 'reactivate', child: Text('Reactivate account')),
      if (user.status != 'DELETED') const PopupMenuItem<String>(value: 'delete', child: Text('Delete account')),
    ],
  );
  ...
  Widget _userCard(MobileManagedUser u) { final locked = u.role == 'SUPER_ADMIN' || me?.id == u.id; return CardBox(margin: const EdgeInsets.only(bottom: 12), child: Row(children: [CircleAvatar(radius: 23, backgroundColor: soft, child: const Icon(Icons.person_outline, color: accent)), const SizedBox(width: 12), Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(u.name, style: const TextStyle(fontWeight: FontWeight.w800, fontSize: 16)), const SizedBox(height: 3), Text(u.email, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(color: muted, fontSize: 12)), const SizedBox(height: 7), Wrap(spacing: 6, runSpacing: 5, children: [StatusPill(u.role), StatusPill(u.status)])])), if (!locked) _menu(u)])); }
}

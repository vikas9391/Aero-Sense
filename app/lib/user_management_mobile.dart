...
class MobileManagedUser {
  ...
  factory MobileManagedUser.fromJson(Map<String, dynamic> j) => MobileManagedUser(
    ...
    name: j['name'] ?? '',
    email: j['email'] ?? '',
    role: (j['role'] ?? 'VIEWER').toString().toUpperCase(),
...
class MobileUserProfile {
  ...
  factory MobileUserProfile.fromJson(Map<String, dynamic> j) => MobileUserProfile(
    ...
    maintenanceCount: j['maintenance_count'] ?? 0,
    componentUpdateCount: j['component_update_count'] ?? 0,
  );
}
...
class _UserManagementScreenState extends State<UserManagementScreen> {
  ...
  Future<void> changeRole(MobileManagedUser user) async {
    ...
    final confirmed = await _confirm(
      ...
      'This account will immediately use ${_roleLabel(role)} permissions on its next authenticated request.',
    );
    if (!confirmed) return;
    try {
      await api.dio.put('/users/${user.id}/role', data: {'role': role});
      await load();
      _message('Role updated to ${_roleLabel(role)}.');
    } catch (e) {
  ...
  Future<void> setStatus(MobileManagedUser user, String status) async {
    ...
    final confirmed = await _confirm(
      '$label ${user.name}?',
      status == 'DELETED'
          ? 'The account will be soft-deleted. Work history and audit records are preserved.'
          : status == 'SUSPENDED'
    ...
    try {
      ...
      await load();
      _message('Account ${status.toLowerCase()}.');
    } catch (e) {
      _message(api.errorMessage(e), error: true);
    }
  }
      ...
      showDialog(
        ...
        builder: (_) => AlertDialog(
          title: Row(children: [
            ...
            Expanded(child: Text(p.user.name)),
          ]),
          content: SingleChildScrollView(
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              _detail('Email', p.user.email),
              _detail('Role', _roleLabel(p.user.role)),
              _detail('Status', p.user.status),
              _detail('Company', p.companyName ?? 'Platform'),
              const Divider(height: 24),
              _detail('Maintenance records', '${p.maintenanceCount}'),
              _detail('Component updates', '${p.componentUpdateCount}'),
            ]),
          ),
  ...
  Widget _detail(String title, String value) => Padding(
    ...
    child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
      SizedBox(width: 130, child: Text(title, style: const TextStyle(color: muted, fontSize: 12))),
  ...
  Widget _menu(MobileManagedUser user) => PopupMenuButton<String>(
    ...
    onSelected: (action) {
      switch (action) {
        case 'profile': viewProfile(user);
        case 'role': changeRole(user);
        case 'suspend': setStatus(user, 'SUSPENDED');
        case 'reactivate': setStatus(user, 'ACTIVE');
        case 'delete': setStatus(user, 'DELETED');
      }
    },
    itemBuilder: (_) => [
      ...
      if (user.status == 'ACTIVE')
        const PopupMenuItem<String>(value: 'suspend', child: Text('Suspend account')),
      if (user.status == 'SUSPENDED')
        const PopupMenuItem<String>(value: 'reactivate', child: Text('Reactivate account')),
      if (user.status != 'DELETED')
        const PopupMenuItem<String>(value: 'delete', child: Text('Delete account')),
    ],
  );
  ...
  Widget build(BuildContext context) => RefreshIndicator(
    ...
    child: ListView(
      ...
      children: [
        ...
        const Text('Users & Roles', style: TextStyle(fontSize: 27, fontWeight: FontWeight.w800)),
        const SizedBox(height: 5),
        const Text('Manage employee access, roles and account status from the mobile app.', style: TextStyle(color: muted, height: 1.4)),
        ...
        if (loading)
          ...
          const CardBox(child: Text('No users found.', style: TextStyle(color: muted)))
        else
  ...
  Widget _userCard(MobileManagedUser u) {
    final self = me?.id == u.id;
    final locked = u.role == 'SUPER_ADMIN' || self;
    return CardBox(
      margin: const EdgeInsets.only(bottom: 12),
      child: Row(children: [
        CircleAvatar(radius: 23, backgroundColor: soft, child: const Icon(Icons.person_outline, color: accent)),
        const SizedBox(width: 12),
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Wrap(spacing: 6, runSpacing: 5, children: [StatusPill(u.role), StatusPill(u.status)]),
        ])),
        if (!locked) _menu(u),
      ]),
    );
...
class _CompanyUsersManagementState extends State<CompanyUsersManagementScreen> {
  ...
  Future<void> load() async {
    if (mounted) setState(() { loading = true; error = null; });
    try {
      final response = await api.dio.get('/companies/${widget.company.id}/users');
      final data = response.data;
      users = data is List
          ? data.map((e) => MobileManagedUser.fromJson(Map<String, dynamic>.from(e))).toList()
          : [];
    } catch (e) {
      error = api.errorMessage(e);
    }
  ...
  Widget _line(String a, String b) => Padding(
    ...
    child: Row(children: [
      SizedBox(width: 115, child: Text(a, style: const TextStyle(color: muted))),
      Expanded(child: Text(b, style: const TextStyle(fontWeight: FontWeight.w700))),
    ]),
  );

  Future<bool> _confirm(String title) async => await showDialog<bool>(
    context: context,
    builder: (_) => AlertDialog(
      title: Text(title),
      content: const Text('This change will take effect immediately.'),
      actions: [
        TextButton(onPressed: () => Navigator.pop(context, false), child: const Text('Cancel')),
        FilledButton(onPressed: () => Navigator.pop(context, true), child: const Text('Confirm')),
      ],
    ...
    final result = await showDialog<String>(
      ...
      builder: (_) => StatefulBuilder(
        builder: (context, set) => AlertDialog(
          ...
          actions: [
            TextButton(onPressed: () => Navigator.pop(context), child: const Text('Cancel')),
            FilledButton(onPressed: () => Navigator.pop(context, role), child: const Text('Save')),
          ],
        ),
      ),
    );
    if (result == null || result == u.role) return;
    final ok = await _confirm('Change ${u.name}\'s role to ${_roleLabel(result)}?');
    if (!ok) return;
    await api.dio.put('/users/${u.id}/role', data: {'role': result});
    await load();
  }

  void _message(String t, {bool error = false}) => ScaffoldMessenger.of(context).showSnackBar(
    SnackBar(content: Text(t), backgroundColor: error ? Colors.red : null),
  );
    ...
    child: ListView(
      ...
      children: [
        CardBox(child: Row(children: [
          ...
          const SizedBox(width: 12),
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text(widget.company.name, style: const TextStyle(fontSize: 20, fontWeight: FontWeight.w800)),
            Text(widget.company.slug, style: const TextStyle(color: muted, fontSize: 11)),
          ])),
          ...
          ...users.map((u) => CardBox(
            ...
            child: Row(children: [
              ...
              Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                Text(u.email, style: const TextStyle(color: muted, fontSize: 12)),

      ...
      child: ListView(
        ...
        children: [
          ...
          if (loading)
            ...
            CardBox(child: Column(children: [const Icon(Icons.business_outlined, size: 42, color: muted), const SizedBox(height: 12), const Text('No companies yet', style: TextStyle(fontWeight: FontWeight.w800)), const SizedBox(height: 5), const Text('Create the first company to get started.', style: TextStyle(color: muted)), const SizedBox(height: 14), FilledButton.icon(onPressed: createCompany, icon: const Icon(Icons.add), label: const Text('Create Company'))]))
          else
      ...
      body: RefreshIndicator(
        ...
        child: ListView(padding: const EdgeInsets.all(20), children: [
          ...
          CardBox(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            else if (users.isEmpty) const Text('No users yet. Use Add Admin from Company Management.', style: TextStyle(color: muted))
            else ...users.map((u) => Padding(padding: const EdgeInsets.only(bottom: 12), child: ListTile(contentPadding: EdgeInsets.zero, leading: CircleAvatar(backgroundColor: soft, child: const Icon(Icons.person_outline, color: accent)), title: Text(u.name, style: const TextStyle(fontWeight: FontWeight.w700)), subtitle: Text(u.email, style: const TextStyle(color: muted)), trailing: StatusPill(u.role)))),
          ])),
        ]),
      ),
    );
...
class _AdminDialogState extends State<_AdminDialog> {
  ...
  Widget build(BuildContext context) => AlertDialog(
    ...
    actions: [TextButton(onPressed: () => Navigator.pop(context), child: const Text('Cancel')), FilledButton(onPressed: () { if (name.text.trim().isEmpty || email.text.trim().isEmpty || password.text.length < 8) return; Navigator.pop(context, [name.text.trim(), email.text.trim(), password.text]); }, child: const Text('Create Admin'))],
  );
}

...
import { ShieldCheck, ScanLine, Wrench, LogOut, RefreshCw, ChevronRight, Plane, Boxes, Home as HomeIcon } from "lucide-react-native";
...
export default function App() {
  ...
  const navItems=[["home","Home",HomeIcon],["components","Parts",Boxes],["aircraft","Fleet",Plane],["scan","Verify",ScanLine]] as const;
  ...
  return <SafeAreaView style={styles.safe}>
    ...
    <View style={styles.nav}>
      {navItems.map(([id,label,Icon])=>
        <TouchableOpacity key={id} onPress={()=>setScreen(id as any)} style={styles.navItem}>
          <View style={[styles.navPill,screen===id&&styles.navPillActive]}><Icon color={screen===id?C.accent:C.muted} size={18}/><Text style={[styles.navText,screen===id&&{color:C.accent}]}>{label}</Text></View>
        </TouchableOpacity>
      )}
    </View>
  </SafeAreaView>;
...
function Scan({onResult,result,back}:{onResult:(r:any)=>void,result:any,back:()=>void}){
 ...
 const trust = result ? (status==="AUTHENTIC" ? 100 : status==="SUSPICIOUS" ? 55 : 15) : 0;
 const statusTone = status==="AUTHENTIC" ? C.good : status==="SUSPICIOUS" ? C.warn : status==="INVALID" ? C.bad : C.accent;
